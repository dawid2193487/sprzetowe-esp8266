...
void WebServer::index() {
	...
	if (digitalRead(led) == 1) {
		response += "<h1>LED is <span class=\"red\">OFF</span></h1>";
	} else {
		response += "<h1>LED is <span class=\"green\">ON</span></h1>";
	}
	
    response += "<form action=\"/\" method=\"post\"><input type=\"submit\" value=\"Przełącz\"></form>";

	server.send(200, "text/html", response);
}

void WebServer::toggle_led() {
}
...
void WebServer::init() {
	...
	server.on("/", WebServer::index);
	server.on("/toggle", WebServer::toggle_led);
